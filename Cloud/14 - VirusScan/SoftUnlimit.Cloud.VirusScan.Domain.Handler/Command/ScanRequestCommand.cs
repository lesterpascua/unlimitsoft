...
namespace SoftUnlimit.Cloud.VirusScan.Domain.Handler.Command
{
        ...
        ICloudCommandHandler<ScanRequestCommand>
    {
        ...
        public async Task<ICommandResponse> HandleAsync(ScanRequestCommand command, CancellationToken ct = default)
        {
            try
            {
                if (_request.Customer is null || _request.Customer.VirusDetected < _options.MaxVirusAttemptPerCustomer)
                {
                    ...
                    await MoveToCompleteAsync(command, StorageStatus.Success, ScanStatus.VirusDetected, true, ct);
        ...
        private async Task<bool> ScanRequest(ScanRequestCommand command, CancellationToken ct)
        {
            ...
            switch (scanStatus)
            {
                ...
                case ScanStatus.VirusDetected:
                    await MarkAttemptAsVirusAsync(command, downloadStatus, scanStatus, ct);
                    break;
                ...
                default:
                    await MarkAttemptAsErrorAsync(command, downloadStatus, scanStatus, ct);
                    break;
            }
            await _unitOfWork.SaveChangesAsync(ct);
            return success;
        ...
        private async Task MarkAttemptAsErrorAsync(ScanRequestCommand command, StorageStatus downloadStatus, ScanStatus scanStatus, CancellationToken ct)
        {
            _logger.LogError("Error scanning file {Id}", _request.Id);

            await MoveToCompleteAsync(command, downloadStatus, scanStatus, true, ct);
        }
        private async Task MarkAttemptAsVirusAsync(ScanRequestCommand command, StorageStatus downloadStatus, ScanStatus scanStatus, CancellationToken ct)
        {
        ...
        private async Task MarkAttemptAsCleanAsync(ScanRequestCommand command, StorageStatus downloadStatus, ScanStatus scanStatus, CancellationToken ct)
        {
            ...
            if (success != false)
            {
                ...
                var (complete, metadata) = await MoveToCompleteAsync(command, downloadStatus, scanStatus, false, ct);

                var documentCreateBody = new DocumentCreateBody(_request.CustomerId, _request.RequestId, _request.BlobUri, metadata);
                complete.AddEvent(typeof(DocumentCreateEvent), command.Props.User.Id, _gen, _request.CorrelationId, documentCreateBody);
            }
            else
            {
        ...
        /// <param name="ct"></param>
        /// <returns></returns>
        private async Task<(Complete, object)> MoveToCompleteAsync(ScanRequestCommand command, StorageStatus downloadStatus, ScanStatus scanStatus, bool deleteBlobUri, CancellationToken ct)
        {
            ...
            complete.AddEvent(typeof(RequestCompleteEvent), command.Props.User.Id, _gen, _request.CorrelationId, body);

...
namespace SoftUnlimit.Cloud.VirusScan.Data.Configuration
{
    public class PendingEntityTypeBuilder : _EntityTypeBuilder<Pending>
    {
        public override void Configure(EntityTypeBuilder<Pending> builder)
        {
            ...
            builder.Property(p => p.Created).HasComment("Date where request is created").HasConversion(DateTimeUtcConverter.Instance);
            builder.Property(p => p.Scheduler).HasComment("Date where the file will be scanned").HasConversion(DateTimeUtcConverter.Instance);
